...
export default function FancyCard({ title, children, github, website, variant = "neutral", deck = false, className = "" }) {
    ...
    return (
        <div className={`${className} relative w-full h-full`}>
            ...
            className={`${base} ${isGradient ? gradient : neutral} ${link ? 'cursor-pointer' : ''}`}
            style={{ willChange: "transform, box-shadow" }}
        >

            {github && (
                <div className={`absolute z-10 top-4 right-4 p-2 rounded-full bg-[#FFD600] shadow-lg border-2 border-white/60 flex items-center justify-center pointer-events-none`}>
